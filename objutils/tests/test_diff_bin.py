...
from objutils.utils.diff import (
    BinDiff,
    diff_bin,
)
...
def test_modify1():
    ...
    res = diff_bin(b0, b1)
    assert res == [
        BinDiff(type='M', offset=126, a=bytearray(b'~\x7f\x80'),
        b=bytearray(b'\x0b\x0c\r')),
        BinDiff(type='M', offset=136, a=bytearray(b'\x88'),
        b=bytearray(b'\x0e'))
    ]
...
def test_modify2():
    ...
    assert res == [
        BinDiff(type='M', offset=20, a=bytearray(b'uvwxy'),
        b=bytearray(b'hello')),
        BinDiff(type='M', offset=45, a=bytearray(b'TUVWX'),
        b=bytearray(b'world'))
    ]
