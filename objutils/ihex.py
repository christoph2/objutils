...
class Reader(hexfile.Reader):
    ...
    def check_line(self, line, format_type):
        ...
        checksum = checksums.lrc(
            utils.make_list(
                line.type, line.length, utils.int_to_array(line.address), line.chunk
            ),
            8,
    ...
    def calculate_extended_address(self, line, shift_by, name):
        if len(line.chunk) == 2:
            ...
            self.error(
                "Bad Extended {0} Address at line #{1}.".format(name, line.line_number)
            )
    ...
    def special_processing(self, line, format_type):
        if line.type == DATA:
            ...
            if len(line.chunk) == 4:
                ...
                self.error(
                    "Bad Segment Address at line %{0:u}.".format(line.line_number)
                )
            ...
            if len(line.chunk) == 4:
                ...
                self.error(
                    "Bad Linear Address at line #{0:d}.".format(line.line_number)
                )
        elif line.type == EOF:
            pass
        else:
            self.warn(
                "Invalid record type [{0:u}] at line {1:u}".format(
                    line.type, line.line_number
                )
            )
...
class Writer(hexfile.Writer):
            ...
            if address > 0xFFFF:
                if address > 0xFFFFF:
                    ...
                    result = ":02000004{0:04X}{1:02X}\n".format(
                        int(seg), self.checksum(list((2, 4, segHi, segLo)))
                    )
                else:
                    seg = int(seg) << 12
                    segHi, segLo = self.word_to_bytes(seg)
                    result = ":02000002{0:04X}{1:02X}\n".format(
                        int(seg), self.checksum(list((2, 2, segHi, segLo)))
                    )
        address = offs
        checksum = self.checksum(list((length, h, l)) + list(row))
        self.previosAddress = offs + length
        result += ":{0:02X}{1:04X}{2:02X}{3!s}{4:02X}".format(
            length, address, DATA, Writer.hex_bytes(row), checksum
        )
        return result
    ...
    def compose_footer(self, meta):
        ...
        return ":00{1:04X}{0:02X}{2:02X}\n".format(
            EOF, self.start_address, self.checksum(list((h, l, EOF)))
        )
